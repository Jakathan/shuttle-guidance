...
//runOncePath("lib_location_constants.ks").
...
function entry_Guidance {
    ...
    local tlast is t0.
    local tnew is t0.
    local psi is site_Bearing(site).
    local xrange_max is 1.5.
    lock downrange to site:distance.

    //Initialize variables for aerodynamic force calculation
